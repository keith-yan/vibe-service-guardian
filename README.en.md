...
Vibe Service Guardian 0.8.4 (unreleased) is a local-first Windows, macOS, and Linux tool for service attribution, AI-runtime health checks, open-weight model capacity planning, and inference optimization. It answers which processes are listening, which project or agent chain they belong to, what a stop would affect, whether a service was relaunched, whether a running local model is healthy and safely exposed, which local models can satisfy a requested workload, and how far measured performance differs from the capacity prediction.

Version 0.8.4 P0 adds bounded 5/15/30-minute post-stop observation, exact hashed historical lifecycle labels, display-only lifecycle-manager guidance, and fixed 60-second concurrency-1/2 calibration for an already loaded local model. Measured profiles separate the theoretical capacity ceiling, measured usable limit, and recommended safe concurrency; profiles are invalidated when the hardware fingerprint or total VRAM changes. VSG still never auto-stops a relaunched process, downloads a model, reads target-process environment variables, or deliberately triggers OOM. See [docs/V0.8.4-P0-CLOSURE.md](docs/V0.8.4-P0-CLOSURE.md) and [docs/PRODUCTION-READINESS-0.8.4.md](docs/PRODUCTION-READINESS-0.8.4.md).
