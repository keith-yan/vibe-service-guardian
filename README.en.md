...
Vibe Service Guardian 0.8.5 (unreleased) is a local-first Windows, macOS, and Linux tool for service attribution, AI-runtime health checks, open-weight model capacity planning, and inference optimization. It answers which processes are listening, which project or agent chain they belong to, what a stop would affect, whether a service was relaunched, whether a running local model is healthy and safely exposed, which local models can satisfy a requested workload, and how far measured performance differs from the capacity prediction.
