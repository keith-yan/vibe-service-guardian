...
[Alpha 1 release notes](docs/RELEASE-NOTES-0.8.5.2-alpha.1.md) · [Impact & evidence](IMPACT.md) · [Roadmap](ROADMAP.md) · [Maintainers](MAINTAINERS.md) · [Governance](GOVERNANCE.md) · [Security](SECURITY.md) · [Privacy](PRIVACY.md)

Vibe Service Guardian **0.8.5.2** uses the unsigned Alpha prerelease identifier **`v0.8.5.2-alpha.1`**. The public `main` branch contains the 0.8.5 P2-A, 0.8.5.1, and 0.8.5.2 P1 work; consult GitHub [Releases](https://github.com/keith-yan/vibe-service-guardian/releases) to determine whether final downloadable assets have been uploaded. This identifier is not a stable-release or complete cross-platform-support claim: the current Windows host has candidate evidence, macOS x86_64 has one constrained automated VMware preview, and Apple Silicon, physical Intel Mac, and Linux target-host acceptance remain outstanding.

VSG is a local-first Windows, macOS, and Linux tool for service attribution, AI-runtime health checks, open-weight model capacity planning, and inference optimization. It answers which processes are listening, which project or agent chain they belong to, what a stop would affect, whether a service was relaunched, whether a running local model is healthy and safely exposed, which local models can satisfy a requested workload, and how far measured performance differs from the capacity prediction.
...
Version 0.8.5 P2-A turns an explicit attribution correction into a scoped, versioned local rule; conflicts resolve by specificity and then the latest user intent, while hits and corrections are counted by unique service episodes. A redacted rule pack requires integrity/conflict preview and explicit rebinding before import. Agent-managed children inherit protection, and Docker collection now requests only a fixed allowlist instead of reading full inspect objects. VSG still never auto-stops, auto-adjusts a rule, syncs a rule pack, or reads target-process/container environment variables. See [docs/V0.8.5-P2-A.md](docs/V0.8.5-P2-A.md) and [docs/PRODUCTION-READINESS-0.8.5.md](docs/PRODUCTION-READINESS-0.8.5.md).

The rule workspace supports search, enable/disable, editing, five retained versions, rollback, hit/override evidence, and a 30-day correction rate whose denominator is unique service lifetimes rather than UI refreshes. `EXPORT RULES` omits local paths, full commands, notes, and non-portable selectors. Import verifies canonical SHA-256, previews candidates/conflicts, and requires the user to bind every selected rule to a current service and scope; there is no automatic cross-machine matching or cloud synchronization.
